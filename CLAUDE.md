...
- Logic lives in `lua/core/theme.lua`: `is_dark()` reads `defaults read -g AppleInterfaceStyle`; `apply()` sets `background` and selects the scheme. **Dark** → **`islands-dark`**; **light** → `catppuccin` **latte** (runs `catppuccin.setup` with `no_italic = true` + integrations for treesitter, LSP, telescope, gitsigns, nvim-tree, which-key, indent-blankline). `apply()` also calls `refresh_lualine()` so the statusline theme follows appearance.
