# Spec: Keymaps — lua/core/mappings.lua
> All global keybindings. Plugin-specific keys live in plugin specs.
...
map("n", "<leader>x", function()
  -- Close buffer without closing window (switch to prev first)
...
- Use `vim.keymap.set(mode, lhs, rhs, opts)` exclusively — never `vim.api.nvim_set_keymap`
- Always include `desc = "..."` — feeds which-key
- LSP keymaps are **buffer-local** and set in `on_attach` in `configs/lspconfig.lua` — do not duplicate them here
- Plugin-specific keys (telescope, lazygit, etc.) belong in `keys = {}` in the plugin spec
