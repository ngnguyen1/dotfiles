...
- `lazy = false`, `priority = 1000` (loaded first).
- `style = 'night'`; `vim.cmd.colorscheme 'tokyonight-night'` on config. Non-italic comments (`styles.comments.italic = false`).
- Single fixed dark theme — no macOS appearance switching or `theme-reload` integration.
...
- `options.theme = 'tokyonight'` (built-in lualine theme matching Tokyonight Night).
- Lazy: `VeryLazy`.
